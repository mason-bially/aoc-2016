...
function ensure_bot(bots, botnum, outputs)
  if botnum < 0
    if -botnum > length(outputs)
      resize!(outputs, -botnum)
    end
    outputs[-botnum] = 0
    return
  end
...
function do_bot(bots, botnum, outputs)
  if botnum != 0 && bot_complete(bots, botnum)
    if bots[botnum].b_low > 0
      ...
      do_bot(bots, bots[botnum].b_low, outputs)
    elseif bots[botnum].b_low < 0
    ...
    if bots[botnum].b_high > 0
      ...
      do_bot(bots, bots[botnum].b_high, outputs)
    elseif bots[botnum].b_high < 0
      outputs[-bots[botnum].b_high] = bots[botnum].v_in2
    end
  end
end

function aoc_10b(input)
  bots = []
  outputs = []
  for line in split(input, '\n')
    ...
    elseif startswith(line, "bot")
      ...
      ensure_bot(bots, bots[botnum].b_low, outputs)
      ensure_bot(bots, bots[botnum].b_high, outputs)
    end
    do_bot(bots, botnum, outputs)
  end
  return outputs[1] * outputs[2] * outputs[3]
end

print(aoc_10b(@aoc_input))
